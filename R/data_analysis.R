...
# Differential abundance guide 5 ------------------------------------------

# Define the KO_g2 vs WT contrast
contrast_matrix <- limma::makeContrasts(
  "KO_g5-WT" = KO_g5-WT,
  "KO_g2-KO_g5" = KO_g2 - KO_g5,
  levels = design_matrix
)
...
ggplot2::ggsave(here::here("plots/volcano_DE_guide_5.pdf"),
                units = "mm",
                height = 60,
                width = 60)
...
ggplot2::ggsave(here::here("plots/protein_LFQs_Ybx1.pdf"),
                units = "mm",
                height = 60,
                width = 60)
...
# Downstream targets of YBX3 ----------------------------------------------

library(decoupleR)
collectri <- get_collectri(organism='human', split_complexes=FALSE)

gene_targets <- collectri |>
  dplyr::filter(source == "YBX3") |>
  dplyr::mutate(downstream = snakecase::to_sentence_case(target)) |>
  dplyr::mutate(downstream = gsub(
    pattern = " ",
    replacement = "",
    downstream
  )) |>
  dplyr::pull(downstream)
...
ggplot2::ggsave(here::here("plots/volcano_YBX3_gene_targets.pdf"),
                units = "mm",
                height = 90,
                width = 90)
